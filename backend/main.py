...
async def alerts_stream(websocket: WebSocket):
    """Stream live alerts to connected clients. Push whenever the background
    scheduler generates a new critical/warning event."""
    await websocket.accept()
    register_ws(websocket)
    try:
        # Send last 10 alerts on connect as a backlog
        for alert in get_recent_alerts(limit=10):
            await websocket.send_json(alert)
        # Keep connection alive; send alerts as they arrive
        while True:
            # Ping every 30s to keep the connection alive and detect drops
            await websocket.receive_text()
    except WebSocketDisconnect:
        pass
    except Exception as e:
        # Log but don't crash; client may have dropped
...
@app.get("/api/alerts")
def get_alerts_endpoint(limit: int = 50, severity: str = None):
    """Recent alerts generated by the background scheduler."""
    return {
        "alerts": get_recent_alerts(limit=limit, severity=severity),
        "count": len(get_recent_alerts(limit=limit, severity=severity)),
    }
...
def acknowledge_alert_endpoint(alert_id: str):
    ...
    return {"acknowledged": acknowledge_alert(alert_id), "alert_id": alert_id}
...
def cohort_endpoint():
    """Vehicle cohort analysis by purchase year — battery health comparison."""
    return cohort_analysis_by_age()


@app.get("/api/analytics/tco-trend")
def tco_trend_endpoint(months: int = 12):
    """Total Cost of Ownership per-km trend over the last N months."""
    return tco_trend(months=months)


@app.get("/api/analytics/vendor-scorecard")
def vendor_scorecard_endpoint():
    """Quarterly supplier scorecard with letter grades."""
    return vendor_scorecard()


@app.get("/api/analytics/carbon-credits")
def carbon_credits_endpoint():
    """Carbon credit marketplace valuation of avoided emissions."""
    return carbon_credit_marketplace()
...
def scheduler_status_endpoint():
    """Background job status — useful for ops dashboards."""
