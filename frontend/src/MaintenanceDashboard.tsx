...
const PRIORITY_BADGE: Record<string, string> = {
    ...
    high: 'bg-status-warning-bg text-orange-800',
    medium: 'bg-status-warning-bg text-yellow-800',
    low: 'bg-voltage-50 text-voltage-700',
};
    ...
    return (
        <div className="mt-12 space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-ink">Maintenance Operations Optimiser</h2>
                ...
                <div className="text-right">
                    <span className="text-xs text-ink-faint uppercase tracking-wider">Schedule Date</span>
                    <p className="text-lg font-semibold text-ink">{shift_date}</p>
                </div>
            </div>

            {/* KPI Cards */}
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                <KPICard label="Total Tasks" value={kpis.total_tasks} color="text-ink" />
                <KPICard label="Scheduled" value={kpis.scheduled_tasks} color="text-status-ok-fg" />
                <KPICard label="Overflow" value={kpis.overflow_tasks} color="text-status-critical-fg" />
                <KPICard label="Delayed (Parts)" value={kpis.delayed_tasks} color="text-status-warning-fg" />
                <KPICard label="Total Cost" value={`₹${kpis.total_cost_inr.toLocaleString('en-IN')}`} color="text-voltage-700" />
                <KPICard label="Critical Same-Day" value={`${kpis.critical_tasks_same_day_pct}%`} color="text-status-ok-fg" />
            </div>

            {/* Secondary KPIs */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-canvas rounded-lg border border-hairline shadow-sm">
                    <p className="text-xs text-ink-faint uppercase tracking-wider mb-1">Avg Wait Time</p>
                    <p className="text-2xl font-bold text-ink">{kpis.avg_wait_hours}h</p>
                    <p className="text-xs text-ink-muted mt-1">From shift start to task begin</p>
                </div>
                <div className="p-4 bg-canvas rounded-lg border border-hairline shadow-sm">
                    <p className="text-xs text-ink-faint uppercase tracking-wider mb-1">Total Fleet Downtime</p>
                    <p className="text-2xl font-bold text-ink">{kpis.total_downtime_hours}h</p>
                    <p className="text-xs text-ink-muted mt-1">Cumulative maintenance hours</p>
                </div>
                <div className="p-4 bg-canvas rounded-lg border border-hairline shadow-sm">
                    <p className="text-xs text-ink-faint uppercase tracking-wider mb-1">Throughput</p>
                    <p className="text-2xl font-bold text-ink">{kpis.throughput_tasks_per_shift}</p>
                    <p className="text-xs text-ink-muted mt-1">Tasks per 8-hour shift</p>
                </div>
            </div>

            {/* Bay Utilization Bars */}
            {!roleView?.hiddenWidgets?.includes('maintenanceSchedule') && (
            <div className="p-5 bg-canvas rounded-lg border border-hairline shadow-sm">
                <h3 className="text-sm font-semibold text-ink uppercase tracking-wider mb-4">Bay Utilization</h3>
                <div className="space-y-3">
                    {['Bay A - Heavy Duty', 'Bay B - General', 'Bay C - General', 'Bay D - Quick Service'].map((name, idx) => (
                        <div key={name} className="flex items-center gap-3">
                            <span className="text-xs text-ink-muted w-36 truncate">{name}</span>
                            <div className="flex-1 bg-canvas-sunken rounded-full h-4 overflow-hidden">
                                <div
                                    className={`h-4 rounded-full ${BAY_COLORS[idx]} transition-all`}
                                    style={{ width: `${kpis.bay_utilization_pct[idx] || 0}%` }}
                                />
                            </div>
                            <span className="text-xs font-medium text-ink w-12 text-right">
            ...
            {!roleView?.hiddenWidgets?.includes('maintenanceSchedule') && (
            <div className="p-5 bg-canvas rounded-lg border border-hairline shadow-sm">
                <h3 className="text-sm font-semibold text-ink uppercase tracking-wider mb-4">Schedule Timeline (Gantt)</h3>

                {/* Time axis */}
                <div className="flex items-center mb-2 ml-36">
                    {Array.from({ length: TOTAL_HOURS + 1 }, (_, i) => (
                        <span key={i} className="text-[10px] text-ink-faint" style={{ width: `${100 / TOTAL_HOURS}%` }}>
                            {SHIFT_START + i}:00
                        </span>
                    ))}
                </div>
                ...
                <div className="space-y-2">
                    {Object.entries(bayGroups).map(([bayName, tasks]) => (
                        <div key={bayName} className="flex items-center gap-2">
                            <span className="text-xs text-ink-muted w-36 truncate font-medium">{bayName}</span>
                            <div className="flex-1 relative bg-canvas rounded h-10 border border-hairline">
                                {tasks.map((task) => {
                ...
                <div className="flex gap-4 mt-4 pt-3 border-t border-hairline">
                    {Object.entries(PRIORITY_COLORS).map(([priority, color]) => (
                        <div key={priority} className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-ink-muted capitalize">{priority}</span>
                        </div>
                    ))}
                </div>
            </div>
                        ...
                        <tbody>
                            {scheduledTasks.map((task) => (
                                <tr key={task.task_id} className="bg-canvas border-b bg-canvas">
                                    ...
                                    <td className="px-3 py-2 text-xs text-ink-faint">{task.spare_parts_needed.join(', ') || '—'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            {(delayedTasks.length > 0 || overflowTasks.length > 0) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {delayedTasks.length > 0 && (
                        <div className="p-5 bg-status-warning-bg rounded-lg border border-status-warning-border">
                            <h3 className="text-sm font-semibold text-status-warning-fg uppercase tracking-wider mb-3">
                                Delayed — Awaiting Parts ({delayedTasks.length})
                            </h3>
                            {delayedTasks.map((task) => (
                                <div key={task.task_id} className="flex justify-between items-center py-2 border-b border-status-warning-border last:border-0">
                                    <div>
                                        <span className="font-medium text-ink text-sm">{task.vehicle_id}</span>
                                        <span className="text-xs text-ink-muted ml-2">{task.task_type}</span>
                                    </div>
                                    <span className="text-xs text-status-warning-fg">Needs: {task.spare_parts_needed.join(', ')}</span>
                                </div>
                            ))}
                        </div>
                    )}
                    {overflowTasks.length > 0 && (
                        <div className="p-5 bg-status-critical-bg rounded-lg border border-status-critical-border">
                            <h3 className="text-sm font-semibold text-status-critical-fg uppercase tracking-wider mb-3">
                                Overflow — Next Day ({overflowTasks.length})
                            </h3>
                            {overflowTasks.map((task) => (
                                <div key={task.task_id} className="flex justify-between items-center py-2 border-b border-status-critical-border last:border-0">
                                    <div>
                                        <span className="font-medium text-ink text-sm">{task.vehicle_id}</span>
                                        <span className="text-xs text-ink-muted ml-2">{task.task_type}</span>
                                    </div>
            ...
            {/* Constraints Summary */}
            <div className="p-4 bg-canvas rounded-lg border border-hairline text-xs text-ink-muted">
                <span className="font-semibold text-ink">Optimization Method: </span>
                {constraints_summary.optimization_method} •
                <span className="ml-2 font-semibold text-ink">Shift Pattern: </span>
                {constraints_summary.shift_pattern} •
                <span className="ml-2 font-semibold text-ink">Priority Levels: </span>
