...
export default function FleetReadinessView({ data }: Props) {
  if (data.length === 0) {
    return (
      <div>
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Fleet Electrification Readiness & Procurement</h2>
        <div className="p-8 bg-white rounded-lg border border-gray-200 shadow-sm text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Ensure the backend is running and generating synthetic fleet data.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Fleet Electrification Readiness & Procurement</h2>
      <p className="text-sm text-gray-500 mb-4">Weighted scoring engine ranking 100 ICE vehicles by EV conversion feasibility.</p>

      {/* Fleet Summary KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
        <KPICard label="Total Fleet" value={data.length} color="text-gray-800" />
        <KPICard label="Ready (Score >70)" value={data.filter(d => d.readiness_score > 70).length} color="text-green-700" />
        <KPICard label="Marginal (40-70)" value={data.filter(d => d.readiness_score > 40 && d.readiness_score <= 70).length} color="text-yellow-600" />
        <KPICard label="Not Ready (<40)" value={data.filter(d => d.readiness_score <= 40).length} color="text-red-600" />
        <KPICard label="Avg TCO Savings" value={`${(data.reduce((a, b) => a + b.tco_savings_pct, 0) / data.length).toFixed(1)}%`} color="text-blue-700" />
        ...
        <table className="w-full text-sm text-left text-gray-500">
          <thead className="bg-gray-50 text-gray-700 uppercase text-xs">
            <tr>
              ...
              <th className="px-3 py-3">Est. Cost (INR)</th>
            </tr>
          </thead>
          <tbody>
            {data.map((item, i) => (
              <tr key={item.vehicle_id} className="bg-white border-b hover:bg-gray-50">
                ...
                <td className="px-3 py-3">
                  <span className="px-2 py-0.5 text-xs font-bold rounded bg-green-100 text-green-800">{item.tco_savings_pct}%</span>
                </td>
