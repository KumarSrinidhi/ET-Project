  ...
  return (
    <>
      <div className="p-6 bg-white shadow-md rounded-lg border border-gray-200">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">EV Asset Performance Management (APM)</h2>
        <p className="text-sm text-gray-500 mb-4">Search fleet health, detect anomalies, schedule maintenance, trace supply chains, analyze quality, and track carbon.</p>
          ...
          <div>
            ...
            {agentData.results && 'kpis' in agentData.results && (agentData.results as any).kpis?.total_emissions_tons_co2 !== undefined && (
              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                ...
                <p className="text-xs text-green-600 mt-3">Switch to the Net Zero tab for full scope breakdown, fleet comparison, and progress tracking.</p>
              </div>
            )}

            {agentData.results.length === 0 && !scheduleData && !mapData && (
              <p className="text-sm text-gray-500 italic">No results returned. Click a preset button above or enter a search query.</p>
            )}
          </div>
        )}
      </div>
